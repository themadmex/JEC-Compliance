...
| # | Topic | Change |
|---|---|---|
| 1 | Auditor access management | Recorded completed implementation: `GET /api/v1/auditors`, `GET /api/v1/audits/{id}/auditors`, scoped_token generation on assignment, portal link, revoke 404 + audit log |
| 2 | Architecture Decision 5.7 | Token-vs-scope separation documented as resolved design decision |
| 3 | users table | Added `scoped_token` column |
| 4 | Phase 5 | Updated Auditor Assignment section to match implementation |
| 5 | API surface | Added two new auditor endpoints |
| 6 | Phase 9 | Added token revocation scope clarification |
| 7 | Phase 10 | Referenced `test_phase6_hardening.py` |
| 8 | Files touched | Recorded touched files for traceability |
...
These are intentionally separate concerns. Implemented in Phase 5 / v2.5.
...
- The portal link shown in the audit detail UI is constructed from the `scoped_token` and the audit ID. It is not a signed URL with embedded scope; scope is always checked server-side.
...
- The portal link displayed in the audit detail UI is constructed from `scoped_token` + `audit_id`; scope is always enforced server-side, not encoded in the link
...
Returns all `audit_users` rows for the audit, including `access_expires_at`, assignment date, and whether the auditor's token is set.
...
- Auditor `scoped_token` is a user-level credential; it proves identity but does not encode scope. Scope is always checked against `audit_users` on every request. Revoking an auditor from one audit does not invalidate their token; it removes their `audit_users` row. See Section 5.7 for full rationale.
- To fully block a revoked auditor who has no remaining assignments: set `users.is_active=false`. This fails the `require_role` check before scope is even evaluated.
...
| Auditor assignment | Wrong-role user -> 422; `access_expires_at` computed correctly |
...
#### Completed Test Files (as of v2.5)
- `tests/test_phase6_hardening.py` - covers auditor assignment, scoped_token generation, portal link construction, revoke 404 behavior, and `auditor.revoked` audit log entry. 9/9 passing.
...
## 11. Implementation Changelog (Files Touched)

This section records which source files were modified in each implementation sprint for traceability. It is updated by the team after each completed feature.

### v2.5 - Auditor Access Management
| File | Change |
|---|---|
| `app/services/user_service.py` | scoped_token generation on first auditor assignment |
| `app/services/audit_service.py` | auditor listing, assignment with token check, revoke with 404 guard |
| `app/routes/audits.py` | new GET /auditors, GET /audits/{id}/auditors routes; updated assignment and revoke routes |
| `app/repository.py` | queries for eligible auditors and audit_users rows |
| `frontend/app.js` | audit detail UI: auditor assignment form, access expiry field, portal link display, revoke button, preview-as-auditor button |
| `tests/test_phase6_hardening.py` | new test file; 9/9 passing |
...
## 14. Engineering Backlog by Phase
...
### Phase 5 - Audit Workflow
- [ ] Audit period creation
- [ ] Audit creation with control defaulting
- [ ] Auditor assignment with expiry
- [ ] Status transition endpoint
- [ ] Auditor scope dependency
- [ ] "View as auditor" preview endpoint
- [ ] Export ZIP generation (streaming)

### Phase 6 - PBC Requests
- [ ] Request creation (auditor + compliance manager)
- [ ] Internal assignment endpoint
- [ ] Evidence attachment endpoint
- [ ] Auditor status update endpoint
- [ ] Threaded comment endpoint
- [ ] Overdue APScheduler job

### Phase 7 - Readiness Engines
- [ ] Type I engine with `readiness_snapshots` and `readiness_gaps` writes
- [ ] Type II engine with gap timeline and snapshot writes
- [ ] Readiness API endpoints
- [ ] Snapshot inclusion in audit export
...
- [ ] Controls inventory
- [ ] Control detail
- [ ] Evidence locker + upload + detail
- [ ] Integrations
- [ ] Tasks
- [ ] Audits overview + audit detail
- [ ] Auditor portal (scoped)
- [ ] PBC requests
- [ ] Reports / export
- [ ] Settings / RBAC
- [ ] Audit log viewer
...
- [ ] `require_audit_scope` FastAPI dependency
- [ ] CSRF middleware
...
## 15. Known Gaps and Future Roadmap
